...
namespace CommonUtilities
{
	...
	public:
		...
		const CommonUtilities::Vector3<T>& GetMinCorner() const;
		const CommonUtilities::Vector3<T>& GetMaxCorner() const;
	private:
		CommonUtilities::Vector3<T> myMinCorner;
		CommonUtilities::Vector3<T> myMaxCorner;
	...
	template<typename T>
	inline const CommonUtilities::Vector3<T>& AABB3D<T>::GetMinCorner() const
	{
		return myMinCorner;
	}
	template<typename T>
	inline const CommonUtilities::Vector3<T>& AABB3D<T>::GetMaxCorner() const
	{
		return myMaxCorner;
	}
}