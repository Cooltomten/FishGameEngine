...
namespace CommonUtilities
{
	...
	public:
		...
		bool GetKeyUp(Keys aKey);
		bool GetKey(Keys aKey);
		POINTS GetMousePosition();
		POINTS GetMousePosDelta();
		float GetMiddleMouseDelta();
		void SetMousePosition(short x, short y);
		void SetLockMouseToWindow(bool aFlag);
	private:
		BYTE myKeys[VIRTUAL_KEY_COUNT];
		BYTE myCurrentKeys[VIRTUAL_KEY_COUNT];
		BYTE myPrevKeys[VIRTUAL_KEY_COUNT];
		POINTS myMousePos;
		POINTS myPrevMousePos;
		POINTS myMousePosDelta;

		float myMiddleMouseDelta;
		float myCurrentMiddleMouseDelta;
	};
}